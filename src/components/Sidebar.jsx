...
const Sidebar = () => {
  return (
    ...
    {data.map((item, index) => {
      const { id, title, subtitle, active } = item;

      return (
        <div className='flex w-[8.4375rem] h-[2.0625rem] flex-shrink-0 mb-5' key={id}>
          {/* INDEX */}
          <div className={`flex w-[2.0625rem] h-[2.0625rem] justify-center items-center mr-4 font-semibold rounded-[50%] ${active ? 'text-denim bg-sky-blue ' : 'text-white border-[1px] border-white'}`}>
            {index + 1}
          </div>
